...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string; taskId: string }> }) {
  try {
    ...
    if (body.action === "update") { const values = Object.fromEntries(Object.entries({ titulo: body.titulo, descricao: body.descricao, prioridade: body.prioridade, responsavel: body.responsavel, prazo: body.prazo }).filter(([,v])=>v!==undefined)); await admin.from("tasks").update({ ...values, updated_at: new Date().toISOString() }).eq("id", taskId); }
    if (body.action === "add_label") await admin.from("task_labels").insert({ task_id: taskId, name: body.labelName, color: body.color || "#0ea5e9" });
    if (body.action === "remove_label") await admin.from("task_labels").delete().eq("id", body.labelId).eq("task_id", taskId);
    if (body.action === "add_member") await admin.from("task_members").upsert({ task_id: taskId, member_name: body.memberName, role: body.role || "responsavel" }, { onConflict: "task_id,member_name" });
    if (body.action === "remove_member") await admin.from("task_members").delete().eq("id", body.memberId).eq("task_id", taskId);
    if (body.action === "add_checklist") { const { data } = await admin.from("task_checklists").insert({ task_id: taskId, title: body.checklistTitle || "Checklist" }).select("id").single(); if (!data) throw new Error("Não foi possível criar o checklist."); }
    if (body.action === "add_checklist_item") await admin.from("task_checklist_items").insert({ task_id: taskId, checklist_id: body.checklistId || null, content: body.content, position: Date.now() });
    if (body.action === "toggle_checklist_item") await admin.from("task_checklist_items").update({ done: body.done, updated_at: new Date().toISOString() }).eq("id", body.itemId).eq("task_id", taskId);
    if (body.action === "add_comment") await admin.from("task_comments").insert({ task_id: taskId, profile_id: profile.id, author_name: profile.nome, content: body.content });
    ...
    await admin.from("task_activity_log").insert({ task_id: taskId, profile_id: profile.id, action_type: body.action, action_detail: "Atualização realizada no card" });
